...
****************************************************************************************/
/* Runtime data arrays */
static Adc_RuntimeGroupType Adc_RuntimeGroups[ADC_MAX_GROUPS];
static Adc_RuntimeHwUnitType Adc_RuntimeHwUnits[ADC_MAX_HW_UNITS];

/* Configuration arrays */
extern const Adc_GroupDefType Adc_GroupConfig[ADC_MAX_GROUPS];
extern const Adc_HwUnitDefType Adc_HwUnitConfig[ADC_MAX_HW_UNITS];
...
static inline Std_ReturnType AdcHw_InitDMA(Adc_HwUnitType HwUnitId);
...
Std_ReturnType AdcHw_Init(Adc_HwUnitType HwUnitId)
{
    ...
    ADC_TypeDef* ADCx = ADC_HW_GET_MODULE(HwUnitId);
    if (ADCx == NULL)
    {
        return E_NOT_OK;
    }
...
Std_ReturnType AdcHw_ConfigureGroup(Adc_HwUnitType HwUnitId, Adc_GroupType GroupId)
{
    ...
    if (AdcHw_ConfigureChannels(HwUnitId, GroupId) != E_OK)
    {
        return E_NOT_OK;
    }
...
Std_ReturnType AdcHw_StartSwConversion(Adc_HwUnitType HwUnitId, Adc_GroupType GroupId)
{
    ...
    /* Check if hardware unit is busy */
    if (AdcHw_IsUnitBusy(HwUnitId) == HW_STATE_SW)
    {
        #if (ADC_ENABLE_QUEUING == STD_ON)
        /* Try to add to queue */
        return AdcHw_AddToQueue(HwUnitId, GroupId);
        #else
        return E_NOT_OK;
        #endif
    }
    ...
    /* Enable interrupts */
    if(Adc_GroupConfig[GroupId].Adc_NbrOfChannel == 1 && 
...
Std_ReturnType AdcHw_StopSwConversion(Adc_HwUnitType HwUnitId, Adc_GroupType GroupId)
{
    ...
    if (Adc_RuntimeHwUnits[HwUnitId].CurrentGroupId != GroupId)
    {
        ...
        return AdcHw_RemoveFromQueue(HwUnitId, GroupId);
        #else
        return E_NOT_OK;
        #endif
    }
...
Std_ReturnType AdcHw_ConfigureChannels(Adc_HwUnitType HwUnitId, Adc_GroupType GroupId)
{
    ...
    if(HwUnitConfig->DMAAvailable ==  ADC_DMA_DISABLE)
    {
        const Adc_ChannelDefType* ChannelConfig = &GroupConfig->Adc_ChannelGroup[0];
        if (AdcHw_ConfigureChannel(ADCx, 
                                  ChannelConfig->Adc_ChannelId,
                                  1,  /* Rank starts from 1 */
                                  ChannelConfig->Adc_ChannelSampTime) != E_OK)
        {
            return E_NOT_OK;
        }
    }
 ...
 * @param[in] Rank Channel rank in conversion sequence
 * @param[in] SampleTime Channel sampling time
 * @return E_OK if successful, E_NOT_OK otherwise
 */
Std_ReturnType AdcHw_ConfigureChannel(ADC_TypeDef* ADCx, 
                                     Adc_ChannelType ChannelId, 
                                     uint8 Rank, 
                                     Adc_SamplingTimeType SampleTime)
{
    /* Validate parameters */
    if (ADCx == NULL_PTR)
    {
    ...
    /* Convert channel ID to hardware channel number */
    uint8 HwChannel = AdcHw_GetHwChannelNumber(ChannelId);
    
    /* Convert sampling time to hardware register value */
    uint32 HwSampleTime = AdcHw_GetHwSampleTime(SampleTime);
    
    /* Configure the channel */
    ADC_RegularChannelConfig(ADCx, HwChannel, Rank, HwSampleTime);
...
void AdcHw_InterruptHandler(Adc_HwUnitType HwUnitId)
{
    ...
    const Adc_GroupDefType* GroupConfig = &Adc_GroupConfig[CurrentGroup];
...
void AdcHw_DmaInterruptHandler(Adc_HwUnitType HwUnitId)
{
    ...
    if (DMA_GetITStatus(ADC_HW_GET_DMA_IT_FLAG(HwUnitId)) != RESET)
    {
        /* Clear interrupt flag */
        DMA_ClearITPendingBit(ADC_HW_GET_DMA_IT_FLAG(HwUnitId));
...
Std_ReturnType AdcHw_AddToQueue(Adc_HwUnitType HwUnitId, Adc_GroupType GroupId)
{
        ...
        AdcHw_IsGroupInQueue(HwUnitId, GroupId) == ADC_INVALID_GROUP_ID) 
    {
        ...
        if(QueueCount == 1) Adc_RuntimeHwUnits[HwUnitId].CurrentGroupId =  GroupId;
        return E_OK;
    }
    return E_NOT_OK;
}
...
Std_ReturnType AdcHw_RemoveFromQueue(Adc_HwUnitType HwUnitId, Adc_GroupType GroupId)
{
    ...
    if(AdcHw_IsGroupInQueue != ADC_INVALID_GROUP_ID) 
    {
...
Adc_GroupType AdcHw_IsGroupInQueue(Adc_HwUnitType HwUnitId, Adc_GroupType GroupId)
{
    if(Adc_RuntimeHwUnits[HwUnitId].QueueHead > Adc_RuntimeHwUnits[HwUnitId].QueueTail)
    {
        ...
        for(Adc_GroupType i = 0; i <= Adc_RuntimeHwUnits[HwUnitId].QueueMaxTail;i++ )
        {
            if(Adc_RuntimeHwUnits[HwUnitId].QueueGroup[i] == GroupId) return i;
        }
    }
...
{
    AdcHw_ResetHwRuntime(Adc_HwUnitType HwUnitId);
}
...
{
    /* Convert to STM32F10x sampling time values */
    switch (SampleTime)
    {
        case 1:  return ADC_SampleTime_1Cycles5;
        case 7:  return ADC_SampleTime_7Cycles5;
        case 13: return ADC_SampleTime_13Cycles5;
        case 28: return ADC_SampleTime_28Cycles5;
        case 41: return ADC_SampleTime_41Cycles5;
        case 55: return ADC_SampleTime_55Cycles5;
        case 71: return ADC_SampleTime_71Cycles5;
        case 239: return ADC_SampleTime_239Cycles5;
        default: return ADC_SampleTime_55Cycles5;
    }
}
...
{
    /* Convert to STM32F10x trigger source values */
    switch (TriggerSource)
    {
        case ADC_TRIGG_SRC_SW:
            return ADC_ExternalTrigConv_None;
        case ADC_TRIGG_SRC_HW:
            return ADC_ExternalTrigConv_T1_CC1;  /* Timer 1 CH1 as example */
                                                    // set default 
        default:
            return ADC_ExternalTrigConv_None;
    }
...
static inline Std_ReturnType AdcHw_ConfigureHwModule(Adc_HwUnitType HwUnitId)
{
    ...
    ADC_InitStructure.ADC_DataAlign = (ADC_RESULT_ALIGNMENT == ADC_ALIGN_RIGHT) ? 
...
{
    #if(ADC_ENABLE_DMA == STD_ON)  
    if (ADC_ENABLE_DMA == Adc_HwUnitConfig[HwUnitId].AdcHw_DMAAvailable)
    {
        /* Get ADC hardware module */
        AdcHw_ConfigureHwModuleGroupDMA(HwUnitId, GroupId);
    }
    else AdcHw_ConfigureHwModuleGroupIT(HwUnitId, GroupId);
    #else 
    AdcHw_ConfigureHwModuleGroupIT(HwUnitId, GroupId);
    #endif
}
#if(ADC_ENABLE_DMA == STD_ON)  
...
static inline Std_ReturnType AdcHw_ConfigureNvic(Adc_HwUnitType HwUnitId)
{
    ...
    /* Configure for mc if have different ADC interrupt handler*/
    // if (HwUnitId == 1)
    // {
    ...
    //     NVIC_InitStructure.NVIC_IRQChannel = ADC1_2_IRQn;
    // }
    /* Configure for mc if have same ADC interrupt handler*/
    if (AdcHw_ValidateHwUnit(HwUnitId))
    {
        NVIC_InitStructure.NVIC_IRQChannel = ADC1_2_IRQn;
    }
...
/**
 * @brief Start next conversion of the group
 * @param[in] HwUnitId ADC hardware unit ID
 * @param[in] GroupId ADC group ID
 * @return void
 */
static void AdcHw_StartNextConversion(Adc_HwUnitType HwUnitId, Adc_GroupType GroupId)
{
    ...
    AdcHw_ConfigureChannel(ADCx, 
                          ChannelConfig->Adc_ChannelId,
                          1,
                          ChannelConfig->Adc_ChannelSampTime);
