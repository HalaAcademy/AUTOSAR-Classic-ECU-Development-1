...
****************************************************************************************/
/* Driver state */
Adc_DriverStateType Adc_DriverState = ADC_STATE_UNINIT;
...
void Adc_Init(const Adc_ConfigType* ConfigPtr)
{
    ...
    for (uint8 i = 0; i < ConfigPtr->NumHwUnits; i++)
    {
        if (AdcHw_Init(i) != E_OK)
        {
            ...
            Adc_DriverState = ADC_STATE_UNINIT;
            return;
        }
    }
    ...
    Adc_DriverState = ADC_STATE_INITIALIZED;
}
...
void Adc_StartGroupConversion(Adc_GroupType Group)
{
    ...
    else
    {
        ...
        if (AdcHw_StartHwConversion(HwUnit, Group) == E_OK)
        {
            Adc_UpdateGroupStatus(Group, ADC_BUSY);
        }
    }
...
void Adc_StopGroupConversion(Adc_GroupType Group)
{
    ...
    {
        /* Hardware triggered conversion */
        if (AdcHw_StopHwConversion(HwUnit, Group) == E_OK)
        {
            Adc_UpdateGroupStatus(Group, ADC_IDLE);
        }
    }
                                                 ...
                                                 Adc_ValueGroupType** PtrToSamplePtr)
{
    ...
    *PtrToSamplePtr = GroupConfig->Adc_ValueResultPtr[GroupConfig->Adc_NbrOfChannel * \
                                                     GroupConfig->Adc_SamplesResultCounter +\
                                                     GroupConfig->Adc_ChannelConversionId];
    return AdcHw_GetGroupRuntimeSampCounter(Group);
}
...
Std_ReturnType Adc_Calibrate(Adc_HwUnitType HwUnit)
{
    ...
    /* Perform calibration */
    ADC_TypeDef* ADCx = ADC_HW_GET_MODULE(HwUnit);
    if (ADCx != NULL)
    {
        /* Start calibration */
...
void Adc_MainFunction(void)
{
    ...
    if (Adc_DriverState != ADC_STATE_INITIALIZED)
    {
        return;
    }
