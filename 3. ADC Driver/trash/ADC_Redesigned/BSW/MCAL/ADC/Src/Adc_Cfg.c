...
static const Adc_ChannelDefType Adc_ChannelGroup1[2] = 
{
    {
        .Adc_ChannelId = 0,                          /* PA0 - ADC1_IN0 */
        .Adc_ChannelSampTime = ADC_SampleTime_55Cycles5
    },
    {
...
static const Adc_ChannelDefType Adc_ChannelGroup2[3] = 
{
    {
...
static Adc_ValueGroupType Adc_Group1_ResultBuffer[2 * 1];          /* 2 channels, 1 sample */
static Adc_ValueGroupType Adc_Group2_ResultBuffer[3 * 1];          /* 3 channels, 1 sample */
static Adc_ValueGroupType Adc_Group3_ResultBuffer[4 * 16];         /* 4 channels, 16 samples */
...
void Adc_TransferComplete_Callback(ADC_TypeDef* ADCx)
{
    ...
    Adc_HwUnitType HwUnit = (ADCx == ADC1) ? 1 : 2;
    
    /* Call hardware interrupt handler */
    Adc_InterruptHandler(HwUnit);
}
...
void Adc_DmaTransferComplete_Callback(DMA_Channel_TypeDef* DMAx_Channely)
{
    ...
    // Adc_HwUnitType HwUnit = (DMAx_Channely == DMA1_Channel1) ? 1 : 2;
    Adc_HwUnitType HwUnit = 1;
    /* Call DMA interrupt handler */
    Adc_DMAInterruptHandler(HwUnit);
}
...
const Adc_GroupDefType Adc_GroupConfig[ADC_MAX_GROUPS] = 
{
    ...
    /* Group 3: Streaming access, one-shot, software trigger */
    {
        ...
        .Adc_StreamNumSamples = 16,
        .Adc_ValueResultPtr = Adc_Group3_ResultBuffer,
        .Adc_ValueResultSize = 64,                   /* 4 channels * 16 samples */
        .Adc_SamplesResultCounter = 0,
...
const Adc_HwUnitDefType Adc_HwUnitConfig[ADC_MAX_HW_UNITS] = 
{
    /* ADC Hardware Unit 1 (ADC1) */
    {
        ...
        .AdcHw_GroupQueueSize = ADC_DEFAULT_QUEUE_SIZE,
        .AdcHw_CurrGroupRegisterId = 0,
        .AdcHw_NbrOfGroupRegister = 0,
        .AdcHW_Nvic = ADC_HW_EOC,                    /* End-of-conversion interrupt */
        .AdcHw_DMAAvailable = ADC_ENABLE_DMA,        /* DMA available */
        .AdcHw_HWTrigger = 0                         /* Hardware trigger disabled */
    },
    
    /* ADC Hardware Unit 2 (ADC2) */
    {
        .AdcHW_UnitId = 2,                           /* ADC2 */
        .AdcHW_Prescale = ADC_CLOCK_PRESCALER,       /* Clock prescaler */
        .AdcHw_QueueEnable = ADC_ENABLE_QUEUING,     /* Queue enabled */
        ...
        .AdcHw_NbrOfGroupRegister = 0,
        .AdcHW_Nvic = ADC_HW_EOC,                    /* End-of-conversion interrupt */
        .AdcHw_DMAAvailable = ADC_DISABLE_DMA,       /* DMA not available */
        .AdcHw_HWTrigger = 0                         /* Hardware trigger disabled */
    }
};
...
*                                 CHANNEL CONFIGURATIONS                               *
****************************************************************************************/
const Adc_ChannelDefType Adc_ChannelConfig[ADC_MAX_CHANNELS] = 
{
    /* Channel 0 - PA0 */
    {
        .Adc_ChannelId = 0,
